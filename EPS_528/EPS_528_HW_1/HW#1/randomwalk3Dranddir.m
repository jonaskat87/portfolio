function [ ] = randomwalk3Dranddir(N, I, T, t, X, Y, Z, D, R, reflecting)
close all
...
moviename = ['randomwalk3Dranddir_', num2str(N), '_', num2str(I), '_',...
...
for i = 2 : T
    ...
    if reflecting
        loc(1, :) = abs(mod(loc(1, :) + Drand .* cos(theta) .* sin(phi) - X,...
            2 * X) - X);
        loc(2, :) = abs(mod(loc(2, :) + Drand .* sin(theta) .* sin(phi) - Y,...
            2 * Y) - Y);
        loc(3, :) = abs(mod(loc(3, :) + Drand .* cos(phi) - Z,...
            2 * Z) - Z);
    else
        loc(1, :) = mod(loc(1, :) + Drand .* cos(theta) .* sin(phi), X);
        loc(2, :) = mod(loc(2, :) + Drand .* sin(theta) .* sin(phi), Y);
        loc(3, :) = mod(loc(3, :) + Drand .* cos(phi), Z);
    end
