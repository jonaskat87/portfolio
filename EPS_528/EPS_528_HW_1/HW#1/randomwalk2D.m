function [ ] = randomwalk2D(N, I, T, t, X, Y, D, R, reflecting)
close all
...
moviename = ['randomwalk2D_', num2str(N), '_', num2str(I), '_',...
...
for i = 2 : T
    ...
    theta = 2 * pi * rand(1, N);
    if reflecting
        loc(1, :) = abs(mod(loc(1, :) + D * cos(theta) - X, 2 * X) - X);
        loc(2, :) = abs(mod(loc(2, :) + D * sin(theta) - Y, 2 * Y) - Y);
    else
        loc(1, :) = mod(loc(1, :) + D * cos(theta), X);
        loc(2, :) = mod(loc(2, :) + D * sin(theta), Y);
    end
