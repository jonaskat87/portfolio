function [] = simulation2(d, height0, height1, t1, dtq, Tend, flag)
...
%scheme-related
% dx = 4e-4; %meters
dx = min(round(sqrt(10) ^ (log10(mean([height0 height1])) - 5), 3), 0.01); %meters
dt0 = 1e-11;
dt = dt0;
...
save emplacementresults.mat all
...
shading interp
xticks(0 : k / scale : k)
xticklabels(num2str(round(t / (60 * 60 * 24 * k), 1) * (0 : k / scale : k)'))
xlabel('time [days]')
yticks(0 : n / scale : n)
...
% end
% figure(4); plot([Tm, Tm], [0, 1], 'b-', 'linewidth', 2);
% 
% grey = [160 160 160] ./ 255;
% green = [0 153 0] ./ 255;
% phi_vec = reshape(phi(:, 101 : end), n * (k - 100), 1);
% T_vec = reshape(T(:,101:end), n * (k - 100), 1);
% figure(4); plot(T_vec, phi_vec, 'o', 'color', grey); hold on;
% figure(4); plot([Tm, Tm], [0, 1], 'k-', 'linewidth', 2);
% 
% xlabel('Temperature [K]'); ylabel('\phi (crystalinity)');
% set(gca, 'fontsize', 14);
% 
% % Wright and Okamura 1977 data
% wright_data=[997.89, 96.175;
% 1019.4, 91.440;
% 1038.6, 84.195;
% 1042.2, 79.688;
% 1051.8, 71.749;
...
% 1137.9, 9.6460];
% figure(4);
% plot(wright_data(:, 1) + 273.15, wright_data(:, 2) ./ 100, 'o', 'color', green, 'markerfacecolor', green);

figure(5)
crust = mean(phi(int64((height - height0) / dx) : end, :), 1);
figure(5); plot(tq / (60 * 60 * 24), crust, 'ro');
xlabel('time [days]'); ylabel('Fraction of solid');
set(gca, 'fontsize', 14);

figure(6)
plot(tq / (60 * 60 * 24), T(1, :) - groundtemp, tq / (60 * 60 * 24), T(end, :) - outsidetemp)

% crust=10*mean(phi(int64((height - height0) / dx) : end, :), 1);
% figure(7);plot(tq, crust, 'ro');
% xlabel('time[s]');ylabel('Crust thickness [m]');
% set(gca,'fontsize',14);
% load Makaopuhi_data.mat Makaopuhi_data_1070
% figure(7);hold on;
% plot(Makaopuhi_data_1070(:,2)*24*60*60,Makaopuhi_data_1070(:,1),'k*');
% legend('simulation','Makaopuhi data');